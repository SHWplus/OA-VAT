...
class ORTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        self.lost_threshold = getattr(params, 'lost_threshold', 0.3)  # 使用getattr获取属性，提供默认值
        self.max_lost_frames = getattr(params, 'max_lost_frames', 10)
        self.keep_last_position = getattr(params, 'keep_last_position', True)

        self.consecutive_lost_frames = 0  # 当前连续丢失帧数
        self.is_lost = False  # 目标是否丢失的标志
    ...
    def track(self, image, info: dict = None):
        ...
        confidence = self._calculate_confidence(pred_score_map, response)

        # 5. 检查目标是否丢失
        if confidence < self.lost_threshold:
            self.consecutive_lost_frames += 1
            if self.consecutive_lost_frames >= self.max_lost_frames:
                self.is_lost = True
                print(f"Frame {self.frame_id}: 目标丢失，置信度 {confidence:.4f}")
        else:
        ...
        if not self.is_lost:
            ...
            # 跟踪失败：可以选择保持最后位置或返回特殊标记
            if self.params.keep_last_position:  # 正确：直接访问类属性
    ...
    def _calculate_confidence(self, score_map, response):
        """计算跟踪置信度"""
        # 方法1：使用响应图的最大值
        ...
        std_response = response.std().item()

        # 方法3：结合多种指标（这里简单使用最大值）
