...
try:
    from torch.amp import custom_fwd, custom_bwd  # new API (supports device_type)

    def _custom_fwd(**kwargs):
        return custom_fwd(**kwargs)

    def _custom_bwd(**kwargs):
        return custom_bwd(**kwargs)
except Exception:  # fallback for older torch (torch.cuda.amp, no device_type kw)
    from torch.cuda.amp import custom_fwd as _cuda_custom_fwd, custom_bwd as _cuda_custom_bwd

    def _custom_fwd(**kwargs):
        kwargs.pop("device_type", None)
        return _cuda_custom_fwd(**kwargs)

    def _custom_bwd(**kwargs):
        kwargs.pop("device_type", None)
        return _cuda_custom_bwd
...
class MSDeformAttnFunction(Function):
    @staticmethod
    @_custom_fwd(device_type="cuda", cast_inputs=torch.float32)
    def forward(
...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    ...
    for lid_, (H_, W_) in enumerate(value_spatial_shapes):
        # N_, H_*W_, M_, D_ -> N_, H_*W_, M_*D_ -> N_, M_*D_, H_, W_
